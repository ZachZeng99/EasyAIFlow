...
EasyAIFlow is a desktop client for local AI coding workflows, focused on Claude CLI/Claude Code integration. It's an Electron + React single-codebase app that also supports a web server runtime. The README and UI are in Chinese; code and comments are in English.
...
- `electron/claudeSpawn.ts` spawns `claude` child processes
...
- Responses are written back to Claude's stdin using `buildClaudeControlResponseLine()` and related builders
- Active Claude processes are tracked in `electron/claudeRunRegistry.ts`

### Event Flow
1. User action → bridge call → Electron IPC or HTTP RPC
2. Backend spawns/resumes Claude CLI process
3. Claude stdout → parsed into `ClaudeStreamEvent` (delta, status, complete, permission-request, plan-mode-request, ask-user-question, etc.)
4. Events delivered to frontend via IPC `claude:event` (desktop) or SSE (web)
...
Each has a pending request registry in the backend, a parser in `claudeControlMessages.ts`, and a response builder that writes back to Claude's stdin.
