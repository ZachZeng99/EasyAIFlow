...
await run('bootstrapHarnessFromSession creates planner/generator/evaluator sessions and scaffold files', async () => {
  ...
  try {
    ...
    assert.equal(result.plannerSessionId, result.rootSessionId);
...
await run('bootstrapHarnessFromSession rejects non-Claude sessions', async () => {
  const tempBase = path.resolve('.tmp-tests');
  await mkdir(tempBase, { recursive: true });
  const tempRoot = await mkdtemp(path.join(tempBase, 'session-harness-bootstrap-provider-'));
  const userDataPath = path.join(tempRoot, 'userData');
  const homePath = path.join(tempRoot, 'home');
  const workspace = path.join(tempRoot, 'workspace');
  const storeFile = path.join(userDataPath, 'easyaiflow-sessions.json');
            ...
            dreams: [
              ...
              },
              {
                ...
                sessions: [
                  {
                    id: 'session-1',
                    title: 'Codex root session',
                    preview: 'Start a new Codex conversation.',
                    timeLabel: '3/27 10:00',
                    updatedAt: Date.now(),
                    provider: 'codex',
                    model: 'gpt-5.4',
  ...
  try {
    ...
    await assert.rejects(
      () => sessionStore.bootstrapHarnessFromSession('session-1'),
      /Harness is currently available only for Claude sessions\./,
    );
  } finally {
    if (previousUserProfile === undefined) {
      delete process.env.USERPROFILE;
